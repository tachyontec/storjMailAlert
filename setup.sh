...
old_upd_int=20 #We need this variable defined in file scope(will be used below)

#Create a function to update data to use it below
function update() {

        #We will save the old price of each parameter to a variable
        ...
        read -p "Receiver email(current-> $old_receiver ):"  receiver
        ...
        read -p "How often to run the check?[minutes] (current-> $old_upd_int ) $ " upd_int
        if ! [ -z $upd_int ];then
        ...
        read -p "Minimum err% to send mail? (current-> $old_min ) " min
        if ! [ -z $min ];then
...
#Check if we already have the script in place
if ! [ -s storjMailAlert ];then
        ...
        mv -f storjMailAlert /bin #Move it to the bin to create the command
        update
        echo "Command 'storjMailAlert' was created"
        echo "run it whenever you want to check your logs"
fi

#Using another script to add the task to cron
